...
export class PersonalMatchAgeComponent implements OnInit , OnDestroy {
  ...
  public dataAgeTracking: CheckboxSelectionItem[];
  ...
  ngOnInit(): void {
    ...
    if(user.age_range?.length > 0) {
      ...
      this.formAge.setItem(this.dataAgeTracking.find(item => item.value == ageRange));
    }
  }

  update(){
    this.isSubmitted = true;
    const _selectedAgeRange = this.formAge.selectedItem;
    const selectedAgeRange = this.dataAge.find(item => item.id == _selectedAgeRange.value);
    ...
    this._qService.updateUserTracking({age_range: [selectedAgeRange.id]});
